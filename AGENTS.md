...
- `custom-install-user.sh` — optional per-machine user setup, run as the user at **image build time** (`Dockerfile`). Anything it writes under `$HOME` (Claude plugins, `npx skills`, dotfile copies) lands in an image layer that the `home` volume shadows at runtime, and Docker seeds a named volume from the image only when that volume is created empty. So `./build` against an existing `home` volume does **not** re-apply those effects to the live home — re-run the commands inside the container by hand, or delete the `home` volume. Only the system-wide (`sudo`) installs here survive a rebuild.
